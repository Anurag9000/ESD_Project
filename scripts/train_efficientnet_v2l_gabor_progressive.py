...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Progressively fine-tune EfficientNet V2-L with a fixed Gabor front-end"
    )
    ...
    parser.add_argument("--output-dir", default="runs/efficientnet_v2_l_gabor_progressive")
...
class GaborEfficientNetV2L(nn.Module):
    def __init__(self, num_classes: int, weights_mode: str, args: argparse.Namespace) -> None:
        super().__init__()
        weights = EfficientNet_V2_L_Weights.DEFAULT if weights_mode == "default" else None
        self.gabor_adapter = GaborAdapter(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.backbone(self.gabor_adapter(x))


def backbone_leaf_modules(model: GaborEfficientNetV2L) -> list[tuple[str, nn.Module]]:
...
def build_phase_plan(total_modules: int, args: argparse.Namespace) -> list[PhaseSpec]:
    ...
    if args.head_epochs > 0:
        phases.append(PhaseSpec(name="head_only_plus_gabor_adapter", unfrozen_backbone_modules=0, epochs=args.head_epochs))
...
def set_progressive_trainability(
    ...
    for param in model.gabor_adapter.parameters():
        param.requires_grad = True
...
def build_optimizer(model: GaborEfficientNetV2L, args: argparse.Namespace) -> AdamW:
    head_modules = [model.gabor_adapter, model.backbone.classifier]
    ...
    backbone_params = [
        param for param in model.backbone.features.parameters() if param.requires_grad and id(param) not in head_ids
    ]
...
def main() -> int:
    ...
    if "plastic" not in train_dataset.class_to_idx:
        print("note: current dataset has no `plastic` class; the Gabor front-end will still bias the model toward texture cues.")

    model = GaborEfficientNetV2L(num_classes=len(train_dataset.classes), weights_mode=args.weights, args=args)
    ...
    metrics = {
        "model_name": "efficientnet_v2_l_with_gabor_adapter",
