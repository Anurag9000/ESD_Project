...
if [[ -z "${RUN_STAMP:-}" ]]; then
  LATEST_RUN=$(ls -td Results/convnextv2_nano_six_classes_* 2>/dev/null | head -1 || true)
  if [[ -n "$LATEST_RUN" ]]; then
    # Extract the timestamp part
    RUN_STAMP=$(basename "$LATEST_RUN" | sed 's/convnextv2_nano_six_classes_//')
...
RUN_ROOT="${RUN_ROOT:-Results/convnextv2_nano_six_classes_${RUN_STAMP}}"
LOG_ROOT="${LOG_ROOT:-logs/convnextv2_nano_six_classes_${RUN_STAMP}}"
