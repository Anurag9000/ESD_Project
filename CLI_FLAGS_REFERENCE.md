...
| `--camera-color-cast-probability` | `0.0` | Legacy compatibility knob. Pink tint is disabled. |
| `--camera-color-cast-strength` | `0.0` | Legacy compatibility knob. Pink tint is disabled. |
| `--camera-color-cast-eval` / `--no-camera-color-cast-eval` | `false` | Kept for compatibility. Pink tint is disabled. |
...
| `--class-mapping` | `""` | JSON string for training-time class merging; the default 3-class pipeline ignores extra physical folders outside the logical taxonomy. |
...
| `ENABLE_RAWACC_REFINEMENT` | `0` | Opt-in flag for the second recursive pass. When unset, the default full pipeline stops after `loss_cleanup` and does not run `val_raw_acc` refinement. |
...
- `run_full_training_pipeline.sh` hardcodes the recursive refinement recipe to `batch-size 320`, `patience 3`, `resume-phase-index 1`, `optimizer adamw`, `sampling-strategy balanced`, `skip-supcon`, `classifier-train-mode full_model`, and `frozen-core-backbone-modules 40` for the default `val_loss` recursive pass. The second `val_raw_acc` recursive pass is now opt-in via `ENABLE_RAWACC_REFINEMENT=1`. By default, full-pipeline runs stop after `loss_cleanup`; recursive candidate scoring still evaluates only the validation split, and the test split is reserved for the final test bundle.
